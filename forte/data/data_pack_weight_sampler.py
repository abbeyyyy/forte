    ...
    def generate_weighted_samples(self, reservoir_size: int):
        ...
        while self._curr_data_pack:
            try:
                ...
                # tid = sent["tid"]
                tag = get_tag(self._curr_data_pack, sent, Token, EntityMention, "ner_type")
            ...
            _curr_data_pack_weight = self.data_pack_weighting_fn(
                self._curr_data_pack,
                sent
            )
            if _curr_data_pack_weight == 0:
                continue
            _curr_data_pack_key = random.random() ** (1 / _curr_data_pack_weight)
            if self._priority_queue is None:
                self._priority_queue = [(_curr_data_pack_key, (sent, self._curr_data_pack, tag))]
                hq.heapify(self._priority_queue)
            elif len(self._priority_queue) < reservoir_size:
                hq.heappush(self._priority_queue, (_curr_data_pack_key, (sent, self._curr_data_pack, tag)))
            else:
                hq.heappop(self._priority_queue)
                hq.heappush(self._priority_queue, (_curr_data_pack_key, (sent, self._curr_data_pack, tag)))
        return list(map(lambda x: (x[1][0], x[1][1], self.data_pack_weighting_fn(x[1][1], x[1][0]), x[1][2]),
